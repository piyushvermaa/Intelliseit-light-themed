...
const Logincard: React.FC<LoginCardProps> = ({ onRegisterClick }) => {
  ...
  const handleLogin = async () => {
    ...
    try {
      ...
      localStorage.setItem('userMail', userCredential.user.email ? userCredential.user.email : '')
      localStorage.setItem('userName', userCredential.user.displayName ? userCredential.user.displayName : '')
  ...
  const handleGoogleLogin = async () => {
    try {
      ...
      localStorage.setItem('login', 'true')
      localStorage.setItem('userMail', result.user.email ? result.user.email : '')
      localStorage.setItem('userName', result.user.displayName ? result.user.displayName : '')
  ...
  return (
    <div className="flex flex-wrap gap-10 items-center justify-center">
      <div
        className="max-w-lg my-10 p-6 backdrop-blur-lg rounded-3xl
          bg-gradient-to-br from-[#33333389] via-[#204a6897] to-transparent
          shadow-lg"
      >
        <div className="flex ">
          <form onSubmit={handleSubmit} className="p-3">
            <h1 className="text-[#fafafa] text-4xl mb-5" style={{ fontFamily: 'cursive', fontStyle: 'italic' }}>
              Login
            </h1>
            ...
            <div className="flex flex-col space-y-5" style={{ maxHeight: '80vh', overflowY: 'auto' }}>
              ...
              <button
                type="submit"
                className="flex-shrink-0 py-3 font-medium text-white bg-indigo-600 hover:bg-indigo-500 rounded-lg border-indigo-500 hover:shadow inline-flex space-x-2 items-center justify-center"
              >
                <span>Login</span>
              </button>
              ...
              <button
                type="button"
                className="w-full group text-center py-3 border flex space-x-2 items-center justify-center border-slate-200 rounded-lg text-slate-700 hover:border-slate-400 hover:text-slate-900 hover:shadow transition duration-150 mb-5"
